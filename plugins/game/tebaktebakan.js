...
const pluginConfig = {
    ...
    category: 'game',
    description: 'Tebak-tebakan lucu dan receh',
    usage: '.tebaktebakan',
    example: '.tebaktebakan',
    isOwner: false,
    isPremium: false,
    isGroup: false,
    isPrivate: false,
...
async function handler(m, { sock }) {
    ...
    if (hasActiveSession(chatId)) {
        ...
        if (session && session.gameType === 'tebaktebakan') {
    ...
    const question = getRandomItem('tebaktebakan.json');
    if (!question) {
        await m.reply('❌ *ᴅᴀᴛᴀ ᴛɪᴅᴀᴋ ᴛᴇʀsᴇᴅɪᴀ*\n\n> Data game tidak tersedia!');
        return;
    }
    
    let text = `🤣 *ᴛᴇʙᴀᴋ-ᴛᴇʙᴀᴋᴀɴ*\n\n`;
    text += `\`\`\`${question.soal}\`\`\`\n\n`;
    ...
    const sentMsg = await sock.sendMessage(chatId, { text, contextInfo: getGameContextInfo('🤣 TEBAK-TEBAKAN', 'Jawab receh!') }, { quoted: m });
    
    createSession(chatId, 'tebaktebakan', question, sentMsg.key, 60000);
...
async function answerHandler(m, sock) {
    ...
    if (!session || session.gameType !== 'tebaktebakan') return false;
