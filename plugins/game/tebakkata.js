...
async function handler(m, { sock }) {
    ...
    if (hasActiveSession(chatId)) {
        ...
        if (session && session.gameType === 'tebakkata') {
    ...
    const question = getRandomItem('tebakkata.json');
    if (!question) {
    ...
    const sentMsg = await sock.sendMessage(chatId, { text, contextInfo: getGameContextInfo('🔤 TEBAK KATA', 'Tebak kata dari petunjuk!') }, { quoted: m });
    
    createSession(chatId, 'tebakkata', question, sentMsg.key, 60000);
...
async function answerHandler(m, sock) {
    ...
    if (!session || session.gameType !== 'tebakkata') return false;
