...
async function handler(m, { sock }) {
    ...
    if (hasActiveSession(chatId)) {
        ...
        if (session && session.gameType === 'tebakepep') {
            const remaining = getRemainingTime(chatId);
            let text = `⚠️ *ᴍᴀsɪʜ ᴀᴅᴀ ɢᴀᴍᴇ*\n\n`;
            text += `> 🎮 Tebak karakter Free Fire ini!\n`;
    ...
    const question = getRandomItem('tebakepep.json');
    if (!question) {
    ...
    let text = `🎮 *ᴛᴇʙᴀᴋ ᴇᴘᴇᴘ*\n\n`;
    text += `> 🔥 Karakter Free Fire apa ini?\n`;
    ...
    const sentMsg = await sock.sendMessage(chatId, {
        image: imageBuffer,
        caption: text,
        contextInfo: getGameContextInfo('🎮 TEBAK EPEP', 'Tebak karakter!')
    }, { quoted: m });
    
    createSession(chatId, 'tebakepep', question, sentMsg.key, 45000);
...
async function answerHandler(m, sock) {
    ...
    if (!session || session.gameType !== 'tebakepep') return false;
