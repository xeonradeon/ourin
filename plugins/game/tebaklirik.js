...
const pluginConfig = {
    ...
    alias: ['lirik', 'lyrics'],
    category: 'game',
    description: 'Tebak kata yang hilang dari lirik lagu',
    usage: '.tebaklirik',
    example: '.tebaklirik',
    isOwner: false,
    isPremium: false,
...
async function handler(m, { sock }) {
    ...
    if (hasActiveSession(chatId)) {
        ...
        if (session && session.gameType === 'tebaklirik') {
    ...
    const question = getRandomItem('tebaklirik.json');
    if (!question) {
        await m.reply('❌ *ᴅᴀᴛᴀ ᴛɪᴅᴀᴋ ᴛᴇʀsᴇᴅɪᴀ*\n\n> Data game tidak tersedia!');
        return;
    }
    
    let text = `🎵 *ᴛᴇʙᴀᴋ ʟɪʀɪᴋ*\n\n`;
    text += `\`\`\`${question.soal}\`\`\`\n\n`;
    ...
    const sentMsg = await sock.sendMessage(chatId, { text, contextInfo: getGameContextInfo('🎵 TEBAK LIRIK', 'Tebak lagu!') }, { quoted: m });
    
    createSession(chatId, 'tebaklirik', question, sentMsg.key, 60000);
...
async function answerHandler(m, sock) {
    ...
    if (!session || session.gameType !== 'tebaklirik') return false;
